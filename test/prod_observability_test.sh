...
want "kamal accessory reboot node-exporter" "deploy does not (re)boot the node-exporter accessory"
want "kamal accessory reboot cadvisor" "deploy does not (re)boot the cAdvisor accessory"
want "OBSERVABILITY_OTLP_BASIC" "deploy does not verify ingestion using the root creds the CI already holds"
want "/streams" "deploy does not query the OpenObserve streams API to confirm metrics landed"
