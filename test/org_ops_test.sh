...
want 'RUNNER_HOST: $[[ inputs.runner_hosts ]]' "runner-provision nao usa parallel:matrix sobre runner_hosts"
want 'PREVIEW_HOST: $[[ inputs.preview_hosts ]]' "preview-provision nao usa parallel:matrix sobre preview_hosts"
want 'KEYSERVER_HOST: $[[ inputs.keyserver_hosts ]]' "keyserver-provision nao usa parallel:matrix sobre keyserver_hosts"
want 'PROD_HOST: $[[ inputs.prod_hosts ]]' "prod-provision nao usa parallel:matrix sobre prod_hosts"

want 'if [ -z "$RUNNER_HOST" ]' "runner-provision sem sentinel de host vazio (matrix com default [""] entraria no caminho ativo)"
want 'if [ -z "$PREVIEW_HOST" ]' "preview-provision sem sentinel de host vazio"
want 'if [ -z "$KEYSERVER_HOST" ]' "keyserver-provision sem sentinel de host vazio"
want 'if [ -z "$PROD_HOST" ]' "prod-provision sem sentinel de host vazio"
